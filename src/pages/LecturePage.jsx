...
import { IoArrowBack, IoArrowUpCircleOutline } from 'react-icons/io5'; 
...
const LecturePage = () => {
    ...
    const [visibleParagraphsCount, setVisibleParagraphsCount] = useState(1);
    ...
    const handleContentClick = () => {
        if (!allParagraphsVisible) {
            setVisibleParagraphsCount(prev => prev + 1);
        }
    };

    useEffect(() => {
        setVisibleParagraphsCount(1);
    }, [lectureId]);
    ...
    }, [lectureId, courseData, reloadCourseData]);
        ...
        >
            ...
            <div className={styles.container}>
                {loading ? (
                    ...
                    <div className={styles.message}><p>{err || 'Лекция недоступна.'}</p></div>
                ) : (
                    <div
                    ...
                    >
                        ...
                        <div className={styles.lectureTextContainer}>
                            {paragraphs.map((text, index) => {
                                if (index > visibleParagraphsCount) {
                                    return null;
                                }
                                const paragraphClass = index < visibleParagraphsCount
                                    ? styles.visible
                                    : styles.preview;

                                return (
                                    <p key={index} className={`${styles.paragraph} ${paragraphClass}`}>
                                        {text}
                                    </p>
                                );
                            })}
